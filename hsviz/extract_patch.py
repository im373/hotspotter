...
def in_depth_ellipse2x2(rchip, kp):
    ...
    con = np.array((('    A', 'B / 2', 'D / 2'),
                    ('B / 2', '    C', 'E / 2'),
                    ('D / 2', 'E / 2', '    F')))
    logger.info("%s", formatting.horiz_string(['A matrix A_Q = ', con]))
    ...
    logger.info("%s", formatting.horiz_string(['A_Q = ', A_Q]))
    ...
    A_33 = np.array(((    A, B / 2),
                     (B / 2,     C)))
    logger.info("%s", formatting.horiz_string(['A_33 = ', A_33]))
    ...
    logger.info("%s", formatting.horiz_string(['x_center = ', x_center]))
    logger.info("%s", formatting.horiz_string(['y_center = ', y_center]))
